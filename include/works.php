...
class works{
    ...
    public function getWorkIdByKeywords($keywords = "") {
        ...
        if($keywords){
            $db = new DBManager();
            $queryK = "SELECT distinct(WK.works_id) FROM works_keywords as WK, keywords as K, works as W  where K.id = WK.keywords_id and K.keyword REGEXP '$keywords' and W.id = WK.works_id order by W.title asc";
